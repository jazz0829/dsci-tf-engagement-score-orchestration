...
class ModelOperation():
    ...
    def get_dummy(self, df_in, categorical_cols, numerical_cols, boolean_cols, label):
        """ create dummies for categorical columns """
        indexers = [StringIndexer(inputCol=column, outputCol=column+'_index').fit(df_in) for column in categorical_cols]
        encoders = [OneHotEncoder(dropLast=False, inputCol=indexer.getOutputCol(), outputCol="{0}_encode".format(indexer.getOutputCol()))
                    for indexer in indexers]
        assembler = VectorAssembler(inputCols=numerical_cols + boolean_cols + [encoder.getOutputCol() for encoder in encoders], outputCol="features")
        pipeline = Pipeline(stages=indexers + encoders + [assembler])
        data = pipeline.fit(df_in).transform(df_in)
        data = data.withColumn('label', F.col(label))
        return data

    def transform(self, df_in):
        """ create dense vector """
        return df_in.rdd.map(lambda r: [Vectors.dense(r['features']), r['label'], r['class_weight'], r['pk']]).toDF(['features', 'label', 'class_weight', 'pk'])
    ...
    def select_and_train(self):
        ...
        boolean_cols = [col[0] for col in training_data.dtypes if col[1].startswith('boolean') and col[0] in features]
        ...
        data = self.get_dummy(data, categorical_cols, numerical_cols, boolean_cols, label)
    ...
    def predict(self, new_data):
        ...
        boolean_cols = [col[0] for col in test_data.dtypes if col[1].startswith('boolean') and col[0] in features]
